...
const getBiographyBlock = (field) => {
  switch (field.type) {
    case 'h1': {
      return <Heading level={1}>{field.text}</Heading>
    }
    case 'h2': {
      return <Heading level={2}>{field.text}</Heading>
    }
    case 'paragraph': {
      return <Text element="p" className={style.paragraph}>{field.text}</Text>
    }
    case 'img': {
    ...
    <div className={style.root}>
      <div className={style.biography}>
        <Container className={style.biographyContent}>
          {BIO[id].map(field => getBiographyBlock(field))}
        </Container>
      </div>
    </div>
  )
